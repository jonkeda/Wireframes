...
This document describes the UIMMD rendering system, which transforms the AST into visual SVG/HTML output with support for multiple themes and interactive features.
...
<svg xmlns="http://www.w3.org/2000/svg" 
     ...
     class="uimmd-container uimmd-theme-${this.theme.name}">
    ${defsContent}
    ${content}
</svg>
        `.trim();
    ...
    private renderContainer(node: LayoutNode): string {
        ...
        if (['Card', 'Panel', 'Header', 'Footer', 'Sidebar'].includes(sectionType)) {
            background = this.renderRectangle(node.bounds, `uimmd-${sectionType.toLowerCase()}`);
        }
...
<g class="${classes}" data-id="${control.id || ''}" data-binding="${control.binding || ''}">
    ...
    private renderTextArea(node: LayoutNode, control: Control): string {
        ...
        return `
<g class="uimmd-textarea" data-id="${control.id || ''}">
    <rect x="${x}" y="${y}" width="${width}" height="${height}" 
          rx="${this.theme.borderRadius}" />
...
<text x="${x}" y="${y + height / 2}" 
    ...
    private renderCheckbox(node: LayoutNode, control: Control): string {
        ...
        return `
<g class="uimmd-checkbox ${isChecked ? 'checked' : ''}" 
   data-id="${control.id || ''}">
    ...
    private renderRadio(node: LayoutNode, control: Control): string {
        ...
        return `
<g class="uimmd-radio ${isSelected ? 'selected' : ''}" 
   data-id="${control.id || ''}">
    ...
    private renderDropdown(node: LayoutNode, control: Control): string {
        ...
        return `
<g class="uimmd-dropdown" data-id="${control.id || ''}">
    <rect x="${x}" y="${y}" width="${width}" height="${height}" 
          rx="${this.theme.borderRadius}" />
    <text x="${x + 12}" y="${y + height / 2}" 
          dominant-baseline="central">
...
<line x1="${x}" y1="${lineY}" x2="${x + width}" y2="${lineY}" 
    ...
    private renderImage(node: LayoutNode, control: Control): string {
        ...
        return `
<g class="uimmd-image" data-id="${control.id || ''}">
    <rect x="${x}" y="${y}" width="${width}" height="${height}" 
    ...
    private renderIconSymbol(icon: string, x: number, y: number): string {
        ...
        return `
<g class="uimmd-icon" transform="translate(${x - 10}, ${y - 10})">
    <path d="${iconPath}" />
</g>
        `.trim();
    }
...
export class TabsRenderer {
    render(node: LayoutNode, component: Component): string {
        ...
        tabs.forEach((tab, index) => {
            ...
            headers.push(`
<g class="uimmd-tab-header ${isActive ? 'active' : ''}" 
   transform="translate(${tabX}, ${y})">
    ...
    <text x="${tabWidth / 2}" y="${tabHeight / 2}" 
        ...
        ${tabElement.attributes.title}
            ...
            if (isActive) {
                contents.push(`
<g class="uimmd-tab-content" transform="translate(${x}, ${y + tabHeight})">
    ${this.renderChildren(tab)}
        ...
        return `
<g class="uimmd-tabs" data-id="${component.attributes.id || ''}">
    <rect x="${x}" y="${y}" width="${width}" height="${height}" 
          class="uimmd-tabs-container" />
    ${headers.join('\n')}
    ${contents.join('\n')}
</g>
        `.trim();
...
export class DataGridRenderer {
    render(node: LayoutNode, component: Component): string {
        ...
        columns.forEach((col, i) => {
            ...
            headerContent += `
<g class="uimmd-datagrid-header-cell" transform="translate(${colX}, ${y})">
    ...
    <text x="12" y="${headerHeight / 2}" dominant-baseline="central">
        ...
        return `
<g class="uimmd-datagrid" data-id="${component.attributes.id || ''}">
    <rect x="${x}" y="${y}" width="${width}" height="${height}" 
          class="uimmd-datagrid-container" />
    ${headerContent}
    <line x1="${x}" y1="${y + headerHeight}" 
          x2="${x + width}" y2="${y + headerHeight}" 
          class="uimmd-datagrid-header-border" />
    ${rowContent}
</g>
        `.trim();
    }
    ...
    private renderPlaceholderRows(
        ...
        for (let row = 0; row < count; row++) {
            ...
            widths.forEach((w, i) => {
                content += `
<g class="uimmd-datagrid-cell" transform="translate(${cellX}, ${rowY})">
...
<line x1="${x}" y1="${rowY + rowHeight}" 
      ...
      class="uimmd-datagrid-row-border" />
            `;
        }
        return content;
    }
...
export class InteractiveRenderer {
    ...
    addInteractivity(svg: SVGElement): void {
        ...
        // Add hover effects
        svg.querySelectorAll('.uimmd-button, .uimmd-input').forEach(el => {
...
describe('SvgRenderer', () => {
    it('should render Button correctly', () => {
        ...
        expect(svg).toContain('class="uimmd-button primary"');
...
*UIMMD Renderer Design v1.0 - 2025*
